...
import { ChipRow, ButtonLink, DenseGrid, DenseCell } from "@/components/ui";
import { Architecture } from "@/components/architecture";
import { BreadcrumbJsonLd } from "@/components/breadcrumb-jsonld";
import { SITE_URL, identity, workforceiq } from "@/data/portfolio";
...
export default function WorkforceIQPage() {
  const url = `${SITE_URL}/projects/${workforceiq.slug}`;

  /**
   * The project as a first-class entity authored BY the Person node.
   *
   * A case study that is only prose is, to a crawler, a page that happens to
   * mention a product name. Declaring it as SoftwareSourceCode with `author`
   * pointing at the canonical @id makes "WorkforceIQ" a thing Pratham built —
   * which is the association a recruiter's search is trying to confirm.
   */
  const projectJsonLd = {
    ...
    description: workforceiq.tagline,
    url,
    codeRepository: workforceiq.repo,
    programmingLanguage: ["TypeScript", "Python", "SQL"],
    runtimePlatform: workforceiq.stack,
    author: { "@id": `${SITE_URL}/#pratham` },
    creator: { "@id": `${SITE_URL}/#pratham` },
  ...
  return (
    <article className="mx-auto max-w-3xl px-5 py-20 sm:py-24">
      ...
      {/* TL;DR — the six facts, as the framed dense grid rather than a bare
          hairline table floating on the page background. */}
      <div className="mt-8">
        <DenseGrid as="dl" cols={2}>
          {[
