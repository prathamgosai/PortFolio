 ...
 *
 * The script is loaded LAZILY — on first interaction with this control, or
 * immediately for a returning visitor who already picked a language. It used to
 * load on mount, which meant every visitor on every route paid for a
 * third-party script, its follow-up requests, and the DOM it injects, to power
 * a control almost nobody touches. It is also a third-party beacon, so not
 * firing it until it is asked for is the better default for EU visitors too.
 *
...
export function LanguagePicker() {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      setLang(current);
      // This visitor already opted in on a previous visit, so the widget has to
      // load for the page to come back translated. Everyone else waits.
  ...
  return (
    ...
    <div className="group relative grid h-9 w-9 place-items-center rounded-full text-muted transition-colors hover:bg-wash hover:text-fg focus-within:text-fg">
      ...
      <select
        ...
        // Start fetching Google's script the moment the control is touched, so
        // it is usually ready by the time a language is actually chosen.
        onPointerDown={loadGoogleTranslate}
